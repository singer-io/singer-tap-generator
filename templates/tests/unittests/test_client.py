...
from {{tap_name}}.exceptions import *


default_config = {
    "base_url": "https://api.example.com",
    "request_timeout": 30,
    "{{ config.auth_config_key if config.auth_config_key else 'auth_token' }}": "dummy_token",
}
...
class TestClient(unittest.TestCase):
    ...
    @patch("{{tap_name}}.client.Client._Client__make_request")
    ...
    @parameterized.expand([
        ["400 error", 400, MockResponse(400), {{config.tap_name}}BadRequestError, "A validation exception has occurred."],
        ["401 error", 401, MockResponse(401), {{config.tap_name}}UnauthorizedError, "The access token provided is expired, revoked, malformed or invalid for other reasons."],
        ["403 error", 403, MockResponse(403), {{config.tap_name}}ForbiddenError, "You are missing the following required scopes: read"],
        ["404 error", 404, MockResponse(404), {{config.tap_name}}NotFoundError, "The resource you have specified cannot be found."],
        ["409 error", 409, MockResponse(409), {{config.tap_name}}ConflictError, "The API request cannot be completed because the requested operation would conflict with an existing item."],
    ])
    ...
    @parameterized.expand([
        ["422 error", 422, MockResponse(422), {{config.tap_name}}UnprocessableEntityError, "The request content itself is not processable by the server."],
        ["429 error", 429, MockResponse(429), {{config.tap_name}}RateLimitError, "The API rate limit for your organisation/application pairing has been exceeded."],
        ["500 error", 500, MockResponse(500), {{config.tap_name}}InternalServerError, "The server encountered an unexpected condition which prevented it from fulfilling the request."],
        ["501 error", 501, MockResponse(501), {{config.tap_name}}NotImplementedError, "The server does not support the functionality required to fulfill the request."],
        ["502 error", 502, MockResponse(502), {{config.tap_name}}BadGatewayError, "Server received an invalid response."],
        ["503 error", 503, MockResponse(503), {{config.tap_name}}ServiceUnavailableError, "API service is currently unavailable."],
    ])
    @patch("time.sleep")
